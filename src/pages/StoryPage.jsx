...
export function StoryPage({ onNavigateToShop }) {
  ...
  const textures = [
    {
      title: 'The Relief Seed & Spiral Stitch',
      ...
      notes:
        'Creates a three-dimensional spiral topology that mirrors the golden-ratio geometry of natural sunflowers.',
    },
    {
      ...
      notes:
        'Tightly wound double-crochet loops wrapped seamlessly around inner copper stems to form rigid, lifelike petals.',
    },
    {
      ...
      notes:
        'High-tension structural crochet providing self-supporting volume without plastic frames or synthetic resins.',
    },
  ];
    ...
    <main className="flex-grow w-full max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-12 md:py-20 space-y-24">
      ...
      >
        ...
        <h1 className="font-display text-4xl md:text-6xl lg:text-7xl text-on-background tracking-tight leading-tight">
          Slow Fashion, <br />
          <span className="text-primary-fixed italic font-normal">Knot by Knot.</span>
        </h1>
        <p className="font-body text-base md:text-lg text-on-surface-variant leading-relaxed">
          We reject the ephemeral speed of mass industrial retail. At **KNOTKARI**, luxury is
        ...
        {[
          {
            ...
            desc: 'Every yarn in our atelier is GOTS-certified unbleached organic cotton, cruelty-free merino wool, or pure mulberry silk sourced from heritage mills.',
          },
          {
            icon: Heart,
            title: 'Unbroken Karigar Provenance',
            desc: 'Each creation is signed with an artisan identification tag linking directly to the individual master karigar who sculpted it over hours of manual stitching.',
          },
          {
            icon: ShieldCheck,
            title: 'Heirloom Longevity',
            desc: 'Our structural knotting techniques ensure your crochet pieces maintain their volume and vibrant botanical hues for generations, backed by our lifetime stitch pledge.',
          },
        ].map((pillar, idx) => {
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-outline-variant/10 pb-6">
          <div>
            <span className="font-label text-xs uppercase tracking-widest text-primary font-semibold">
              Interactive Studio Study
            </span>
            <h2 className="font-display text-2xl md:text-3xl text-on-background mt-1">
              The Loom Gallery: Macro Karigari Studies
            </h2>
          </div>
          <div className="flex gap-2">
            {textures.map((t, idx) => (
              <button
                ...
                className={`px-3.5 py-2 rounded-lg text-xs font-label uppercase tracking-wider transition-colors ${
                  activeTexture === idx
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-6 relative aspect-[4/5] rounded-2xl overflow-hidden bg-surface-variant border border-outline-variant/20 shadow-2xl">
            <img
          ...
          <div className="md:col-span-6 space-y-6">
            ...
            <div className="space-y-3 text-xs bg-surface-container p-5 rounded-xl border border-outline-variant/15">
              <div>
                <span className="text-on-surface-variant font-label uppercase tracking-wider block mb-1">
                  Material Integrity:
                </span>
                <span className="text-on-surface font-semibold text-sm">
                  {textures[activeTexture].yarn}
                </span>
              </div>
            ...
            <button
              ...
              className="px-6 py-3.5 bg-on-background hover:bg-primary-fixed text-background hover:text-on-primary-fixed font-label text-xs uppercase tracking-widest font-semibold rounded-xl flex items-center gap-2 transition-colors"
            >
